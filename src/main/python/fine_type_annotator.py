...
def distance_filter(doc, ent, trigger_tuples, max_distance=5):
    ent_start, ent_end = ent.start, ent.end

    def is_close(trigger_tuple):
        (trigger_start, trigger_end), _ = trigger_tuple
        return not (trigger_start < ent_start - max_distance or
                    trigger_end > ent_end + max_distance)

    return filter(is_close, trigger_tuples)


def quotation_filter(doc, ent, trigger_tuples):
    ent_start, ent_end = ent.start, ent.end

    def is_quote_in_middle(trigger_tuple):
        (trigger_start, trigger_end), _ = trigger_tuple
        for i in xrange(ent_start+1, trigger_start):
...
class RuleBasedFineTypeAnnotator(object):
    ...
    def __call__(self, doc):
        ...
        fine_ent_view = []
        for ent in doc.ents:
            filtered_trigger_tuples = quotation_filter(doc, ent, distance_filter(doc, ent, nsd_view))
            ...
            if len(fine_types) > 0:
                fine_ent_view.append(((ent.start, ent.end), fine_types))
...
if __name__ == '__main__':
    ...
    def create_pipeline(nlp):
        return [nlp.tagger, nlp.entity, nlp.parser, NounSenseAnnotator(nsd), RuleBasedFineTypeAnnotator(finetyper)]
